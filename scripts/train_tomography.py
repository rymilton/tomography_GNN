...
class MuonDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Features
        features = torch.tensor([muon[col] for col in self.feature_columns], dtype=torch.float)
...
def muon_group_collate_fn(batch, voxel_densities, num_voxels, device, num_muons_per_group):
    """
    ...
    if total_number_of_muons < total_batch_length:
        ...
        features_list.extend([pad_feature] * (total_batch_length-total_number_of_muons))
    X = torch.stack(features_list).view(num_groups, num_muons_per_group, feature_dim).to(device)
    ...
    voxel_mask = torch.zeros(num_groups, num_muons_per_group, num_voxels, dtype=torch.bool, device=device)
    path_length = torch.zeros(num_groups, num_muons_per_group, num_voxels, dtype=torch.float, device=device)
    ...
    y = torch.stack([torch.tensor(voxel_densities, dtype=torch.float, device=device) for _ in range(num_groups)])
...
def main():
    ...
    max_num_muons = flags.num_muons if flags.num_muons is not None else len(data['dataframe'])
    muon_dataframe, voxel_densities = data['dataframe'][:max_num_muons], data['voxel_densities']
    ...
    num_muons_per_batch = num_muons_per_group * num_muon_groups_per_batch  # total muons per DataLoader batch

    dataset_train, dataset_validation = train_test_split(dataset, test_size=config["VALIDATION_SPLIT"])
    ...
    dataloader_train = DataLoader(
        ...
        collate_fn=lambda b: muon_group_collate_fn(
            b, voxel_densities, num_voxels=MAX_NUMBER_OF_VOXELS, device=device, num_muons_per_group=num_muons_per_group
        )
    )
